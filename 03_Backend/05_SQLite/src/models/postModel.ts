...
export async function fetchAllPosts(req: Request) {
    const db = getDB();
    const posts = await db.all<Post[]>("SELECT * FROM blog_entries");
...
export async function fetchPost(id: string) {
    const db = getDB();
    const [post] = await db.all<Post[]>("SELECT * FROM blog_entries WHERE id = ?", [id]);
    return {
    post: { ...post, createdAt: formatDate(post?.createdAt || undefined) },
  };
}

export async function insertPost(entry: Post): Promise<number> {
  const db = getDB();
...
export async function updatePostById(id: number, entry: Post): Promise<void> {
  const db = getDB();
  console.log(entry);
  await db.run(
    `UPDATE blog_entries
